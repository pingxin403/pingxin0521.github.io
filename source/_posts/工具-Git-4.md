...
*这篇是将本地机做服务器机，设置客户端免密获取或推送到本地上的仓库。*
...
这里的服务器是指做代码托管的代理，使用ssh连接，需要配置秘钥进行无秘登录，不然每次获取都需要输入密码。
...
4. 设置SSH key
   ...
   收集客户端公钥，复制到服务器上。 客户端当前账号`RSA`或`DSA`的数字签名文件`id_rsa.pub` 在`.ssh`用户目录中。通常情况下windows在 `%userprofile%\.ssh` 目录中，Mac在`~/.ssh`目录中。

   win7上可以打开Git GUI界面有个`帮助`->`show SSH Key`菜单，如果没创建过可以点击创建SSH的签名文件，并复制公钥保存到服务器上。如果使用第三方的SSH Key服务，那可能实际位置会有些差异。也可以使用命令行生成SSH Key， `ssh-keygen -t rsa -C "your email"`。

   生成本地用户的签名文件（位于C:\Users\HP\\.ssh\id_rsa.pub），并把客户端上公钥导入到服务器上，将本地机生成的公钥上传到用户家目录下的`.ssh/authorized_keys`文件。如果有多个的话可以放在一个目录中，方便导入。如果是团队人比较多这种方式可能就不太适合了。

   

   修改`/etc/ssh/sshd_config`的配置，保存并重启 

   ```shell
   [git@localhost ~]$ su root
   [root@localhost hyp]# vi /etc/ssh/sshd_config
   [root@localhost hyp]# systemctl restart sshd
   [root@localhost hyp]# su - git
   [git@localhost ~]$ sudo chmod 700 .ssh/
   [git@localhost ~]$ sudo chmod 600 .ssh/*
   ```
   ...
   测试，在客户端上使用以下命令测试是否连接成功：

   ```
   git clone git@<host ip>:/home/git/sample.git
   ```

5. 安全

   为安全考虑Git账号只允许使用`git-shell`。在passwd文件中找到git用户，把`/bin/bash`直接修改成`/usr/bin/git-shell` 登录root账号，并修改git的用户权限。
